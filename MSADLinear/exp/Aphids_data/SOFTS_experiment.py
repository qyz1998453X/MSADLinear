"""
SOFTS 对照实验 - multi-horizon forecasting
保存：
1) 汇总结果 -> result/SOFTS/SOFTS_Result.txt
2) 箱线图逐点误差 -> result/SOFTS/SOFTS_BoxData.csv
"""
...
def main():
    ...
    out_dir = project_root / "result" / "SOFTS"
    ...
    pbar = tqdm(total=total_iters, desc="SOFTS experiment", ncols=100)
    ...
    # ---------- Save ----------
    with open(out_dir / "SOFTS_Result.txt", "w", encoding="utf8") as f:
        f.write("========== 总结 ==========\n")
        f.write("\n".join(summary_lines))

    pd.DataFrame(boxplot_rows).to_csv(out_dir / "SOFTS_BoxData.csv", index=False)
