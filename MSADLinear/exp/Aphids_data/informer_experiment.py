"""
Informer 对照实验 - Fixed model, multi-horizon forecasting
保存：
1) 汇总结果 -> result/Informer/Informer_Result.txt
2) 箱线图逐点误差 -> result/Informer/Informer_BoxData.csv
...
def main():
    ...
    out_dir = project_root / "result" / "Informer"
    ...
    pbar = tqdm(total=total_iters, desc="Informer experiment", ncols=100)
    ...
    # ---------- Save ----------
    with open(out_dir/"Informer_Result.txt","w",encoding="utf8") as f:
        f.write("========== 总结 ==========\n")
        f.write("\n".join(summary_lines))

    pd.DataFrame(boxplot_rows).to_csv(out_dir/"Informer_BoxData.csv",index=False)
