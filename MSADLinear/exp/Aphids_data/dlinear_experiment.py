"""
DLinear 对照实验 - Fixed model, multi-horizon forecasting
保存：
1) 汇总结果 -> result/DLinear/DLinear_Result.txt
2) 箱线图逐点误差 -> result/DLinear/DLinear_BoxData.csv
...
def main():
    ...
    cfg = {
        ...
        "horizons":[1,3,5],
        "epochs":80,
        "individual":False,  # Shared linear layers for all channels
        "kernel_size":3,  # Adjusted for short sequences (seq_len=7)
        "lr":0.0005,
        "batch":16,
        "train_ratio":0.6,
    ...
    out_dir = project_root / "result" / "DLinear"
    ...
    pbar = tqdm(total=total_iters, desc="DLinear experiment", ncols=100)
    ...
    # ---------- Save ----------
    with open(out_dir/"DLinear_Result.txt","w",encoding="utf8") as f:
        f.write("========== 总结 ==========\n")
        f.write("\n".join(summary_lines))

    pd.DataFrame(boxplot_rows).to_csv(out_dir/"DLinear_BoxData.csv",index=False)
