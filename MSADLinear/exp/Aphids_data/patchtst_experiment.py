"""
PatchTST 对照实验 - Fixed model, multi-horizon forecasting
保存：
1) 汇总结果 -> result/PatchTST/PatchTST_Result.txt
2) 箱线图逐点误差 -> result/PatchTST/PatchTST_BoxData.csv
...
def main():
    ...
    cfg = {
        ...
        "patch_len":4,  # Adjusted for short sequences
        "stride":2,  # Adjusted for short sequences
        "lr":0.0005,
        "batch":16,
        "train_ratio":0.6,
    ...
    out_dir = project_root / "result" / "PatchTST"
    ...
    pbar = tqdm(total=total_iters, desc="PatchTST experiment", ncols=100)
    ...
    # ---------- Save ----------
    with open(out_dir/"PatchTST_Result.txt","w",encoding="utf8") as f:
        f.write("========== 总结 ==========\n")
        f.write("\n".join(summary_lines))

    pd.DataFrame(boxplot_rows).to_csv(out_dir/"PatchTST_BoxData.csv",index=False)
