"""
TCN 对照实验 - Fixed model, multi-horizon forecasting
保存：
1) 汇总结果 -> result/TCN/TCN_Result.txt
2) 箱线图逐点误差 -> result/TCN/TCN_BoxData.csv
...
def main():
    ...
    cfg = {
        ...
        "horizons":[1,3,5],
        "epochs":80,
        "num_channels":[32, 32, 32],  # TCN的通道数，与LSTM的hidden=32对应
        "kernel_size":2,
        "lr":0.0005,
        "batch":16,
        "train_ratio":0.6,
    ...
    out_dir = project_root / "result" / "TCN"
    ...
    pbar = tqdm(total=total_iters, desc="TCN experiment", ncols=100)
    ...
    # ---------- Save ----------
    with open(out_dir/"TCN_Result.txt","w",encoding="utf8") as f:
        f.write("========== 总结 ==========\n")
        f.write("\n".join(summary_lines))

    pd.DataFrame(boxplot_rows).to_csv(out_dir/"TCN_BoxData.csv",index=False)
