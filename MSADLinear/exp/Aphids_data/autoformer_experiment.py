"""
Autoformer 对照实验 - Fixed model, multi-horizon forecasting
保存：
1) 汇总结果 -> result/Autoformer/Autoformer_Result.txt
2) 箱线图逐点误差 -> result/Autoformer/Autoformer_BoxData.csv
...
def main():
    ...
    cfg = {
        ...
        "horizons":[1,3,5],
        "epochs":80,
        "d_model":32,  # 与LSTM的hidden_size对齐，保证公平对比
        "n_heads":4,
        "e_layers":2,
        "d_layers":1,
        "d_ff":128,  # Feed-forward dimension
        "moving_avg":3,  # Adjusted for short sequences
        "lr":0.0005,
        "batch":16,
        "train_ratio":0.6,
    ...
    out_dir = project_root / "result" / "Autoformer"
    ...
    pbar = tqdm(total=total_iters, desc="Autoformer experiment", ncols=100)
    ...
    # ---------- Save ----------
    with open(out_dir/"Autoformer_Result.txt","w",encoding="utf8") as f:
        f.write("========== 总结 ==========\n")
        f.write("\n".join(summary_lines))

    pd.DataFrame(boxplot_rows).to_csv(out_dir/"Autoformer_BoxData.csv",index=False)
