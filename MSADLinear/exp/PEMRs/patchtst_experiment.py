"""
PEMRs PatchTST experiment (disease & pest)
...
- Outputs: result2/PatchTST/{Result.txt, Summary.csv, BoxData.csv}
"""
...
def main():
    ...
    out_dir = project_root / "result2" / "PatchTST"
    ...
    pbar = tqdm(total=total_iters, desc="PEMRs PatchTST", ncols=100)
    ...
    for fname, target_col, ds_tag in datasets:
        ...
        for sc in scenarios:
            ...
            for h in cfg["horizons"]:
                ...
                pbar.set_description(f"PatchTST [{ds_tag} {sc} h={h}]")
                    
                model = PatchTSTModel(input_size=Xtr.shape[2], output_size=1, seq_len=cfg["seq_len"])
                model = train_model(
                    model,
    ...
    pd.DataFrame(summary_rows).to_csv(out_dir / "PatchTST_Summary.csv", index=False)
    pd.DataFrame(box_rows).to_csv(out_dir / "PatchTST_BoxData.csv", index=False)

    with open(out_dir / "PatchTST_Result.txt", "w", encoding="utf-8") as f:
        f.write("PEMRs PatchTST summary\n")
