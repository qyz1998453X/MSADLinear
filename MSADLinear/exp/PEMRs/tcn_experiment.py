"""
PEMRs TCN experiment (disease & pest)
...
- Outputs: result2/TCN/{Result.txt, Summary.csv, BoxData.csv}
"""
...
def main():
    ...
    out_dir = project_root / "result2" / "TCN"
    ...
    pbar = tqdm(total=total_iters, desc="PEMRs TCN", ncols=100)
    ...
    for fname, target_col, ds_tag in datasets:
        ...
        for sc in scenarios:
            ...
            for h in cfg["horizons"]:
                ...
                pbar.set_description(f"TCN [{ds_tag} {sc} h={h}]")
                    
                model = TCN(input_size=Xtr.shape[2], output_size=1, num_channels=[32, 32, 32])
                model = train_model(
                    model,
    ...
    pd.DataFrame(summary_rows).to_csv(out_dir / "TCN_Summary.csv", index=False)
    pd.DataFrame(box_rows).to_csv(out_dir / "TCN_BoxData.csv", index=False)

    with open(out_dir / "TCN_Result.txt", "w", encoding="utf-8") as f:
        f.write("PEMRs TCN summary\n")
