"""
PEMRs Autoformer experiment (disease & pest)
...
- Outputs: result2/Autoformer/{Result.txt, Summary.csv, BoxData.csv}
"""
...
from model.autoformer import AutoformerModel
...
def main():
    cfg = {
        "seq_len": 10,  # Autoformer对seq_len>10使用FFT路径有bug，用10走简化注意力
        "horizons": [1, 3],
        "epochs": 80,
        "lr": 0.0005,
        "batch": 16,
    ...
    out_dir = project_root / "result2" / "Autoformer"
    ...
    pbar = tqdm(total=total_iters, desc="PEMRs Autoformer", ncols=100)
    ...
    for fname, target_col, ds_tag in datasets:
        ...
        for sc in scenarios:
            ...
            for h in cfg["horizons"]:
                ...
                pbar.set_description(f"Autoformer [{ds_tag} {sc} h={h}]")
                    
                model = AutoformerModel(input_size=Xtr.shape[2], output_size=1, seq_len=cfg["seq_len"])
                model = train_model(
                    model,
    ...
    pd.DataFrame(summary_rows).to_csv(out_dir / "Autoformer_Summary.csv", index=False)
    pd.DataFrame(box_rows).to_csv(out_dir / "Autoformer_BoxData.csv", index=False)

    with open(out_dir / "Autoformer_Result.txt", "w", encoding="utf-8") as f:
        f.write("PEMRs Autoformer summary\n")
