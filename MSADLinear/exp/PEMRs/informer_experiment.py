"""
PEMRs Informer experiment (disease & pest)
...
- Outputs: result2/Informer/{Result.txt, Summary.csv, BoxData.csv}
"""
...
from model.informer import InformerModel
...
def main():
    ...
    out_dir = project_root / "result2" / "Informer"
    ...
    pbar = tqdm(total=total_iters, desc="PEMRs Informer", ncols=100)
    ...
    for fname, target_col, ds_tag in datasets:
        ...
        for sc in scenarios:
            ...
            for h in cfg["horizons"]:
                ...
                pbar.set_description(f"Informer [{ds_tag} {sc} h={h}]")
                    
                model = InformerModel(input_size=Xtr.shape[2], output_size=1, seq_len=cfg["seq_len"])
                model = train_model(
                    model,
    ...
    pd.DataFrame(summary_rows).to_csv(out_dir / "Informer_Summary.csv", index=False)
    pd.DataFrame(box_rows).to_csv(out_dir / "Informer_BoxData.csv", index=False)

    with open(out_dir / "Informer_Result.txt", "w", encoding="utf-8") as f:
        f.write("PEMRs Informer summary\n")
    ...
    avg_mae = np.mean([row['MAE'] for row in summary_rows])
    print(f"\nseq_len={cfg['seq_len']} 平均MAE: {avg_mae:.3f}")
