"""
PEMRs SOFTS experiment (disease & pest)
...
- Outputs: result2/SOFTS/{Result.txt, Summary.csv, BoxData.csv}
"""
...
def main():
    ...
    out_dir = project_root / "result2" / "SOFTS"
    ...
    pbar = tqdm(total=total_iters, desc="PEMRs SOFTS", ncols=100)
    ...
    for fname, target_col, ds_tag in datasets:
        ...
        for sc in scenarios:
            ...
            for h in cfg["horizons"]:
                ...
                pbar.set_description(f"SOFTS [{ds_tag} {sc} h={h}]")
                    
                model = SOFTS(input_dim=Xtr.shape[2], pred_horizon=1, patch_len=4, d_model=32)
                model = train_model(
                    model,
    ...
    pd.DataFrame(summary_rows).to_csv(out_dir / "SOFTS_Summary.csv", index=False)
    pd.DataFrame(box_rows).to_csv(out_dir / "SOFTS_BoxData.csv", index=False)

    with open(out_dir / "SOFTS_Result.txt", "w", encoding="utf-8") as f:
        f.write("PEMRs SOFTS summary\n")
