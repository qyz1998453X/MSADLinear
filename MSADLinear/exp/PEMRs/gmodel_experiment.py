"""
PEMRs GModel experiment (disease & pest)
...
- Scenarios:
    ...
    target_only    : only target (pure time series)
- Split: chronological 70% train / 30% test
- Horizons: 1, 3
- Seq len: 14
- Metrics: MAE, RMSE, SMAPE
- Outputs: result2/GModel/{Result.txt, Summary.csv, BoxData.csv}
"""
...
def load_excel(path: Path, target_name: str):
    ...
    df = df.sort_values(df.columns[0])  # sort by date
...
def main():
    ...
    out_dir = project_root / "result2" / "GModel"
    ...
    pbar = tqdm(total=total_iters, desc="PEMRs GModel", ncols=100)
    ...
    for fname, target_col, ds_tag in datasets:
        ...
        for sc in scenarios:
            ...
            for h in cfg["horizons"]:
                ...
                # PEMRs数据集：使用GModelPEMRs（可学习分解+Individual模式）
                pbar.set_description(f"GModelPEMRs [{ds_tag} {sc} h={h}]")
                ...
                mae, rmse, smape_val, records = evaluate(
                    model,
    ...
    pd.DataFrame(summary_rows).to_csv(out_dir / "GModel_Summary.csv", index=False)
    pd.DataFrame(box_rows).to_csv(out_dir / "GModel_BoxData.csv", index=False)

    with open(out_dir / "GModel_Result.txt", "w", encoding="utf-8") as f:
        f.write("PEMRs GModel summary\n")
